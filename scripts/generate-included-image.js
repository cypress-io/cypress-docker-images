...
const formIncludedWorkflow = (included) => {
  const yml =
    '      - build-included-image:\n' +
    `          name: "included ${included.tag}"\n` +
    `          dockerTag: "${included.tag}"\n`
  return yml
}

const formAwsCodeBuildIncludedWorkflow = (included) => {
  const identifier = camelCase(`${included.name}${included.tag}`)
  ...
  env:
    ...
    variables:
      IMAGE_REPO_NAME: "cypress/${included.name}"
      IMAGE_DIR: "${included.name}"
      IMAGE_TAG: "${included.tag}"\n`
  return job
}

const writeConfigFile = (included) => {
  const includedWorkflow = formIncludedWorkflow(included)
  const text = preamble.trim() + os.EOL + includedWorkflow
  fs.writeFileSync('circle.yml', text, 'utf8')
  console.log('generated circle.yml')
}

const writeBuildSpecConfigFile = (included) => {
  const includedWorkflow = formAwsCodeBuildIncludedWorkflow(included)
  console.log('** includedWorkflow **', includedWorkflow)
  const text = awsCodeBuildPreamble.trim() + os.EOL + includedWorkflow + os.EOL + awsCodeBuildPostamble.trim()
  console.log('** text **', text)
  fs.writeFileSync('buildspec.yml', text, 'utf8')
  console.log('generated buildspec.yml')
}

const included = splitImageFolderName(outputFolder)
console.log('** included image **')
console.log(included)

writeConfigFile(included)
writeBuildSpecConfigFile(included)
